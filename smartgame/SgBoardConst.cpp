...
SgComponentIterator::SgComponentIterator(const SgBoardConst& boardConst,
                                         const SgSimpleSet& points)
    : m_points(points),
      m_nextPoint(boardConst.FirstBoardPoint()),
      m_boardConst(boardConst)
{
}

bool SgComponentIterator::Next(SgList<SgPoint>* component)
{
    // Skip all points until finding one that's not marked. Use a sentinel
    // to speed up loop by avoiding test for end.
    SgPoint sentinel = m_boardConst.LastBoardPoint()+1;
    SG_ASSERT(! m_points.Contains(sentinel) && m_nextPoint <= sentinel);
    m_points.Include(sentinel);
    while (! m_points.Contains(m_nextPoint))
        ++m_nextPoint;
    m_points.Exclude(sentinel);

    // Return false if done.
    if (sentinel <= m_nextPoint)
        return false;

    // Otherwise, find the component at m_nextPoint. The visited part of the
    // component is unmarked so it won't be found again.
    // AR: could create a class that contains both stack[] and a[] to make
    //       these traversals easier.
    SgPoint p = m_nextPoint;
    int n = 0;
    SgStack<int,SG_MAXPOINT> stack;
    ...
    {
        a[n++] = p;
        // Find neighboring points that are part of the same component.
        if (m_points.Contains(p - SG_NS))
        {
            m_points.Exclude(p - SG_NS);
            stack.Push(p - SG_NS);
        }
        if (m_points.Contains(p - SG_WE))
        {
            m_points.Exclude(p - SG_WE);
            stack.Push(p - SG_WE);
        }
        if (m_points.Contains(p + SG_WE))
        {
