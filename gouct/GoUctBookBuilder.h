...
inline GoBookState::GoBookState(const GoBoard& brd)
    : m_synchronizer(brd)
{
    m_synchronizer.SetSubscriber(m_brd[0]);
}

inline GoBookState::~GoBookState()
{
}

inline GoBoard& GoBookState::Board()
{
    return m_brd[0];
}

inline const GoBoard& GoBookState::Board() const
{
    return m_brd[0];
}

inline SgHashCode GoBookState::GetHashCode() const
{
    return m_hash;
}

inline void GoBookState::Synchronize()
{
...
inline void GoBookState::Play(SgMove move)
{
...
inline void GoBookState::ComputeHashCode()
{
    m_hash = m_brd[0].GetHashCodeInclToPlay();
    for (int rot = 1; rot < 8; ++rot)
    {
...
public:
    ...
    bool Get(const GoBookState& state, SgBookNode& node);

    void Put(const GoBookState& state, const SgBookNode& node);
...
inline GoAutoBook::GoAutoBook(const std::string& filename) throw()
    : m_filename(filename)
{
...
inline GoAutoBook::~GoAutoBook()
{
}

inline bool GoAutoBook::Get(const GoBookState& state, SgBookNode& node)
{
...
inline void GoAutoBook::Put(const GoBookState& state, const SgBookNode& node)
{
    m_data[state.GetHashCode()] = node;
}

inline void GoAutoBook::Flush()
{
    Save(m_filename);
}

inline void GoAutoBook::Save(const std::string& filename) const
{
