...
void GoUctGlobalSearchState::Execute(SgMove move)
{
    GoUctState::Execute(move);
    if (IsInPlayout())
    {
...
SgMove GoUctGlobalSearchState::GenerateRandomMove(bool& skipRaveUpdate)
{
    SG_ASSERT(IsInPlayout());
    ...
    // passes in a row, both of them generated by GenerateRandomMove() in
    // the playout phase
    if (move == SG_PASS)
    {
